 ...
 */
export function createViewListItemElement( writer, type ) {
	const viewItem = writer.createContainerElement( type );
...
/**
 * Helper function that creates a `<dl><dd></dd></dl>` structure out of the given `modelItem` model.
 * Then, it binds the created view list item (`<dd>`) with the model `descriptionListItem` element.
 * The function then returns the created view list item (`<dd>`).
 *
 ...
 */
export function generateItemInDl( modelItem, conversionApi, type ) {
	const mapper = conversionApi.mapper;
	const viewWriter = conversionApi.writer;
	const listType = 'dl';
	const viewItem = createViewListItemElement( viewWriter, type );
 ...
 * should be in a view list element (`<dl>` or `<dl>`) and should be its only child.
...
export function injectViewList( modelItem, injectedItem, conversionApi, model ) {
	...
	if ( refItem && refItem.getAttribute( 'listIndent' ) == modelItem.getAttribute( 'listIndent' ) ) {
		...
		if ( prevItem && prevItem.name == 'descriptionListItem' ) {
			...
			// a nested list directly inside the dd element.
	...
	if ( prevItem && prevItem.name == 'descriptionListItem' ) {
		...
		for ( const value of walker ) {
			if ( value.item.is( 'element', 'dd' ) || value.item.is( 'element', 'dt' ) ) {
		...
		if ( nextViewList && ( nextViewList.is( 'element', 'dl' ) || nextViewList.is( 'element', 'dl' ) ) ) {
...
export function mergeViewLists( viewWriter, firstList, secondList ) {
	...
	if ( !firstList || !secondList || ( firstList.name != 'dl' && firstList.name != 'dl' ) ) {
		return null;
	}
...
export function getSiblingListItem( modelItem, options ) {
	...
	while ( item && item.name == 'descriptionListItem' ) {
...
/**
 * Returns an array with all `descriptionListItem` elements that represent the same list.
 *
 * It means that values of `listIndent`, `listType`, `ListProperties`, `listReversed` and `listStart` for all items are equal.
 *
...
export function getSiblingNodes( position, direction ) {
	const items = [];
	const descriptionListItem = position.parent;
	const walkerOptions = {
	...
	const limitIndent = descriptionListItem.getAttribute( 'listIndent' );
	const nodes = [ ...new TreeWalker( walkerOptions ) ]
		.filter( value => value.item.is( 'element' ) )
		.map( value => value.item );

	for ( const element of nodes ) {
		// If found something else than `descriptionListItem`, we're out of the list scope.
		if ( !element.is( 'element', 'descriptionListItem' ) ) {
			break;
		}
		...
		if ( element.getAttribute( 'listType' ) !== descriptionListItem.getAttribute( 'listType' ) ) {
			break;
		}
		...
		if ( element.getAttribute( 'ListProperties' ) !== descriptionListItem.getAttribute( 'ListProperties' ) ) {
			break;
		}

		// ... different direction
		if ( element.getAttribute( 'listReversed' ) !== descriptionListItem.getAttribute( 'listReversed' ) ) {
			break;
		}

		// ... and different start index
		if ( element.getAttribute( 'listStart' ) !== descriptionListItem.getAttribute( 'listStart' ) ) {
			break;
		}
...
/**
 * Returns an array with all `descriptionListItem` elements in the model selection.
 *
...
export function getSelectedListItems( model ) {
	...
	// and update the `ListProperties` attribute in those lists.
	let listItems = [ ...document.selection.getSelectedBlocks() ]
		.filter( element => element.is( 'element', 'descriptionListItem' ) )
		.map( element => {
...
function getListItemFillerOffset() {
	const hasOnlyLists = !this.isEmpty && ( this.getChild( 0 ).name == 'dl' || this.getChild( 0 ).name == 'dl' );
